module "CoreApp" {
  ...
  stack_name = "64bit Amazon Linux 2 v3.2.10 running Corretto 11"
  ...
  # private_subnets = setunion([data.aws_subnet.private_subnet_1.id, data.aws_subnet.private_subnet_2.id])
  # private_subnets = join(", ", [data.aws_subnet.private_subnet_1.id, data.aws_subnet.private_subnet_2.id])
